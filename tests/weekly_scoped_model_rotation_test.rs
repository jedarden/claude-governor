...
fn test_weekly_scoped_model_rotation_resets_samples() {
    ...
    assert_eq!(
        burn_rate_state.fleet_pct_ema_samples, 10,
        "Should start with 10 samples"
    );
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.weekly_scoped, 2.5,
        "Should start with Fable's EMA"
    );
    assert_eq!(
        burn_rate_state.usd_per_pct_ema_weekly_scoped, 3.2,
        "Should start with Fable's USD EMA"
    );
    ...
    let reset_performed =
        state::reset_weekly_scoped_on_model_change(&prev_model, &new_model, &mut burn_rate_state);
    ...
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.five_hour, 1.8,
        "five_hour EMA should remain unchanged"
    );
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.seven_day, 2.0,
        "seven_day EMA should remain unchanged"
    );
    assert_eq!(
        burn_rate_state.usd_per_pct_ema_five_hour, 2.8,
        "five_hour USD EMA should remain unchanged"
    );
    assert_eq!(
        burn_rate_state.usd_per_pct_ema_seven_day, 3.0,
...
fn test_model_cleared_to_none_resets_samples() {
    ...
    assert!(
        reset_performed,
        "Reset should be performed when model is cleared"
    );
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.weekly_scoped, 0.0,
...
fn test_model_initialization_does_not_reset() {
    ...
    assert!(
        reset_performed,
        "Initialization should be logged as a change"
    );
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.weekly_scoped, 0.0,
...
fn test_no_reset_when_model_unchanged() {
    ...
    assert!(
        !reset_performed,
        "No reset should occur when model is unchanged"
    );
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.weekly_scoped, original_ema,
        "weekly_scoped EMA should remain unchanged when model is unchanged"
    );
    assert_eq!(
        burn_rate_state.usd_per_pct_ema_weekly_scoped, original_usd_ema,
...
fn test_cold_start_uses_baseline_not_zero() {
    ...
    assert!(
        baseline.pct_per_worker_per_hour > 0.0,
        "Baseline pct per worker should be non-zero"
    );
    assert!(
        baseline.dollars_per_worker_per_hour > 0.0,
    ...
    assert!(
        is_cold_start,
        "Should detect cold-start when samples < 3 and no fresh rate"
    );
    ...
    if is_cold_start && util > 0.0 && current_workers > 0 {
        ...
        assert_eq!(
            fleet_pct_hr,
            baseline.pct_per_worker_per_hour * 5.0,
...
fn test_comprehensive_model_rotation_scenario() {
    ...
    assert_eq!(
        poll_result_fable.weekly_scoped_model,
    ...
    assert_eq!(
        poll_result_opus.weekly_scoped_model,
        Some("Opus".to_string())
    );
    ...
    assert!(
        reset_performed,
        "Reset should be performed on Fable -> Opus rotation"
    );
    ...
    assert!(
        poll_result_opus.weekly_scoped_utilization > 0.0,
...
fn test_model_rotation_preserves_other_windows() {
    ...
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.five_hour, five_hour_before,
        "five_hour EMA should be unchanged"
    );
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.seven_day, seven_day_before,
        "seven_day EMA should be unchanged"
    );

    assert_eq!(
        burn_rate_state.usd_per_pct_ema_five_hour, usd_five_hour_before,
        "five_hour USD EMA should be unchanged"
    );
    assert_eq!(
        burn_rate_state.usd_per_pct_ema_seven_day, usd_seven_day_before,
    ...
    assert_ne!(
        burn_rate_state.fleet_pct_hr_ema.weekly_scoped, weekly_scoped_before,
    ...
    assert_ne!(
        burn_rate_state.usd_per_pct_ema_weekly_scoped, usd_weekly_scoped_before,
...
fn test_production_path_identity_change_cold_start_flow() {
    ...
    assert_eq!(
        std_pct_hr_seeded, fleet_pct_hr_seeded,
...
fn test_full_cycle_model_rotation_resets_calibrated_slot() {
    ...
    assert_eq!(
        burn_rate_state.fleet_pct_ema_samples, num_samples_to_calibrate,
    ...
    assert_eq!(
        burn_rate_state.fleet_pct_hr_ema.weekly_scoped, 12.5,
