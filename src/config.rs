...
pub fn find_retired_references(raw: &serde_yaml::Value) -> Vec<String> {
    ...
    for (agent_key, agent_value) in agents {
        let Some(agent_name) = agent_key.as_str() else { continue };
...
"#,
        ...
        assert_eq!(violations.len(), 1, "one key, one violation: {violations:?}");
...
"#,
        ...
        assert!(violations[0].contains("cgov-polish-queue"), "{violations:?}");
    }
...
"#,
        ...
        assert!(violations.len() >= 3, "expected key + name + fields, got: {violations:?}");
        assert!(
