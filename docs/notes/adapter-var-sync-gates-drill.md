...
sandboxed `$HOME` (a copied `claude-print` under `$HOME/.cargo/bin`,
since the installer rejects symlinked candidates and the box's
`~/.local/bin/claude-print` is a symlink) so its adapter installs
...
Re-run signature-for-signature on the same day (same bead, second attempt)
after the first attempt's session died post-commit: all four mutations
reproduced every message below against the committed state, and the M1
rust-side collateral list was corrected — a Rust-side addition fails three
collateral checks, not one (see Mutation 1).
...
(A Rust-side addition fails **three** collateral checks, all because no
committed template or expectation unsets the new variable yet — the scrub
side of the world now demands something nothing provides:

- `adapter_verify::tests::committed_adapter_templates_scrub_both_variable_sets`
  (src/adapter_verify.rs:552)
- `doctor::tests::adapter_check_passes_scrub_only_when_live_is_skipped`
  (src/doctor.rs:3013) — `assertion left == right failed: left: Fail, right: Pass`
- `doctor::tests::adapter_check_runs_the_live_probe_when_asked`
  (src/doctor.rs:3042) — same `left: Fail, right: Pass`

So a rust-side `cargo test --lib` shows 4 failures, not 2. A bash-side edit
produces none of this collateral — exactly the sync test fails.)
...
separate gate. Its exact message:

```text
thread 'sync_gate_flags_a_removed_bash_variable' panicked at tests/adapter_var_sync.rs:159:9:
assertion `left == right` failed: ANTHROPIC_SMALL_FAST_MODEL is not an element of RULE5_API_VARS — pick a variable the array actually declares
  left: 9
 right: 8
```
