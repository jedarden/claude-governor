...
- The `polish-opus` agent's `max_workers: 4` in `governor.yaml` predates this incident and was not itself changed by this ADR — it remains the ceiling that data-driven scale-up can reach once real samples justify it. Whether 4 is still the right ceiling given current weekly-usage patterns is a separate, operator-facing config question, not a code-correctness one.
  - **Update 2026-09-16:** the `polish-opus` pool was retired along with the polish queue, its timer and seeder, and the subscription generator pool (see CLAUDE.md, "Retired 2026-09-16"). A governor.yaml that still names it now **fails config validation at load** (`RETIRED_REFERENCE_MARKERS` in `src/config.rs`), so this ceiling question is moot — the pool cannot be configured at all. The ADR's `current_total`-hold policy remains in force for the pools that do exist. The ADR-002 narrative above is kept as history.
